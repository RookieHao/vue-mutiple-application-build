...
function clean(moduleList,pageModule){
  rimraf(path.resolve(process.cwd(),'dist',pageModule || ''),()=>{
    runBuild(moduleList,pageModule)
  })
}

function runBuild(moduleList,pageModule){
  let buildList
  if (!moduleList) {
...
module.exports = function build(moduleList,pageModule,args){
  if(args){
    opt[args](moduleList,pageModule)
  }else{
    runBuild(moduleList,pageModule)
  }
}
